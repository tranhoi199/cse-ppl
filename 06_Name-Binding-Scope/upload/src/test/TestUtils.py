...
from NameExercise  import NameExercise, RedeclaredException
...
class TestExercise:
    ...
    def test(input,expect,num):
        try:
            ex = NameExercise().visit(input, None)
            return ex == expect
        except RedeclaredException as f:
            return f.message == expect.message
